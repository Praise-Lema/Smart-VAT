...
<div class="main-bar">
    <div class="container">
        ...
        <div class="container ms-2 d-flex justify-content-between">
...
{{-- @dd($sales) --}}
            @if(count($sales) > 0)
                @foreach ($sales as $sale)
                    {{-- Sales Card --}}
                        <div class="card py-2 m-2 shadow" style="width:48%">
                            <div class="card-body">
                                <div class="top-sub-container d-flex justify-content-between">
                                    <div class="amount">
                                        <p class="card-text">Total Sales VAT</p>
                                        <h1 class="card-title">TZS {{number_format($sale->total_sales_vat, 2)}}</h1>
                                    </div>
                                    
                                    <div class="">   
                                        <h5>Month</h5>
                                        <p class="card-text">{{$sale->Month}}</p>
                                        {{-- <p class="card-text">{{date('F',strtotime($sale->Month))}}</p> --}}
                                    </div>
                                </div>

                                <div class="d-flex p-2">
                                    {{-- Total Inclusive sales card --}}
                                    <div class="card shadow w-50 mx-1">
                                        <div class="card-body">
