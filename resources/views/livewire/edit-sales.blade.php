<div class="container my-4 bg-secondary-subtle rounded p-4">
    <h2 class="py-2 text-center">Edit Sales Detail</h2>
    {{ html()->form('PUT','/sales/'.$sale->id)->open()}}
        ...
        {{ html()->div()->class('form-group my-4')->open()}}
            ...
            {{ html()->select('Month')->attributes(['name'=>'Month', 'class'=>'form-select'])->open() }}  
                ...
                {{html()->option($sale->Month)->value("$sale->Month")}}
            {{html()->select()->close()}}
        {{ html()->div()->close() }}


        {{ html()->div()->class('form-group my-4')->open()}}
            {{ html()->submit('Edit')->class('btn btn-dark bg-gradient form-control') }}
        {{ html()->div()->close() }}
